...
namespace XUnitTestTicketSystemPrototypeTest.Test_Model
{
    public class Test_Creating_Event_Valid_input
    {
        ...
        public void Creating_Event()
        {
            ...
            //Wrong
            var EventInput = new Event("", new DateTime(1999, 1, 1), "", new DateTime(1999, 1, 1), 0, "");
            var EventCorrect = new Event("Åpen dag", new DateTime(2020, 1, 1), "Skogen 3b, 1708 Halden", new DateTime(2005, 1, 1), 200, "");

            if (name != "" && date >= DateTime.Now && adress != "" && ageLimit >= new DateTime(2005, 1, 1))
            {
                //Right
                EventInput = new Event(name, date, adress, ageLimit, ticketPrice, eventInfo);
            }
